...
# @@File             :  min.sh
# @@Description      :  Script to setup min for Fedora
...
RELEASE_TYPE="$(. /etc/os-release 2>/dev/null; [[ " $ID_LIKE " == *centos* ]] && echo "fedora")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
SERVICES_DISABLE="avahi-daemon.service avahi-daemon.socket cups.path cups.service cups.socket \
dhcpd dhcpd6 dm-event.socket import-state.service irqbalance.service iscsi iscsid.socket \
iscsiuio.socket kdump loadmodules.service lvm2-lvmetad.socket lvm2-lvmpolld.socket lvm2-monitor \
mdmonitor multipathd.service multipathd.socket named nfs-client.target nis-domainname.service nmb \
radvd rpcbind.service rpcbind.socket smb sssd-kcm.socket timedatex.service tuned.service udisks2.service"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__rm_repo_files() {
	[ "${1:-$YUM_DELETE}" = "yes" ] && rm -Rf "/etc/yum.repos.d"/* &>/dev/null || return 0
}
...
if type -P systemmgr >/dev/null 2>&1; then
	...
	__run_external "dnf clean all -q"
fi
...
# __install_pkg cronie-noanacron  # skipped on fedora
__install_pkg postfix
__install_pkg net-tools
__install_pkg wget1-wget
__install_pkg curl
__install_pkg git
...
__run_external "dnf upgrade -y -q"
##################################################################################################################
...
# __install_pkg cockpit-pcp  # skipped on fedora
# __install_pkg cockpit-bridge  # skipped on fedora
# __install_pkg cockpit-system  # skipped on fedora
# __install_pkg cockpit-ws  # skipped on fedora
...
__install_pkg grub2-efi-x64
__install_pkg grub2-efi-x64-modules
__install_pkg grub2-tools
__install_pkg grub2-tools-extra
__install_pkg shim-x64
...
# __install_pkg python3-enum34  # skipped on fedora
# __install_pkg python3-funcsigs  # skipped on fedora
# __install_pkg python3-future  # skipped on fedora
__install_pkg python3-idna
# __install_pkg python3-josepy  # skipped on fedora
# __install_pkg python3-mock  # skipped on fedora
